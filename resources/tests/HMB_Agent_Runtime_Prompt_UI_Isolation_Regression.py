...
@contextmanager
def _native_parent_fixture():
    if agent._BuiltinAgent is not None:
        yield
        return

    # Public CI uses the import-only DataNode fallback, not the installed
    # Standard Agent. Supply its native parameter store and prompt converter;
    # all HMB getter/setter, private-echo, and publication guards stay real.
    def get_native_value(self, name):
        if name in self.parameter_values:
            return self.parameter_values[name]
        parameter = self.parameters.get(name)
        return getattr(parameter, "default_value", None)

    def set_native_value(self, name, value, *, initial_setup=False, **_kwargs):
        parameter = self.parameters[name]
        candidate = value.strip() if name == "prompt" and isinstance(value, str) else value
        final = candidate if initial_setup else self.before_value_set(parameter, candidate)
        self.parameter_values[name] = final

    with patch.object(agent._BaseAgent, "get_parameter_value", get_native_value), patch.object(
        agent._BaseAgent, "set_parameter_value", set_native_value
    ):
        yield
