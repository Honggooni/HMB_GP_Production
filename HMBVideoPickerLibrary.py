...
def _video_frame_domain(frame_metadata: Any) -> Dict[str, Any]:
    """Return the bounded frame-addressing contract used by Prompt bindings."""
...
def _emitter_identity_from_cue(
    ...
    """Resolve one cue emitter without converting unrelated metadata to intent."""

    source = value if isinstance(value, dict) else {}
    marker_color = _clean(
        source.get("marker_color") or source.get("color")
    )
    ...
    if matched is None and marker_color and not asset_id:
        color_matches = [
            ...
            and _clean(marker.get("color")).casefold()
            == marker_color.casefold()
        ]
        if len(color_matches) == 1:
            matched = color_matches[0]

    # Picker can validate an emitter only through one of its actual marker
    # instances. An arbitrary UUID/path supplied in a generic row is not
    # promoted into a creative emitter binding.
    if matched is None:
        return {}
    ...
    # A cue must identify an emitter by a marker address or stable Maya
    # instance. An asset name alone is intentionally insufficient because it
    # can refer to several instances of the same referenced rig.
...
def _normalize_emitter_local_point(value: Any) -> Dict[str, Any]:
    ...
    if kind == "locator":
        locator_id = _clean(
            value.get("locator_id") or value.get("maya_uuid")
        )[:256]
        locator_path = _clean(
...
def _normalize_timing_cues(
    ...
    for raw in value[:MAX_VIDEO_TIMING_CUES]:
        ...
        if abs(frame_value - rounded_frame) > 1e-6:
            # Prompt/Agent exact cue contracts address discrete source frames.
            # A fractional timestamp may remain descriptive timing input, but
            # Picker must not promote it into an exact emitter-frame cue.
            continue
        ...
        if not local_point:
            # Emitter identity locates an object, not the exact local point on
            # that object. Without an explicit locator or local/object-space
            # coordinate, this row cannot become an exact emitter cue.
            continue
        cue_phase = _clean(
...
def _video_reference_capabilities(
    ...
    """Describe typed support without choosing a Prompt interpretation role."""
...
def _normalize_markers(value: Any, video_slot: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    for index, raw in enumerate(value, start=1):
        ...
        # Asset ID names a reusable asset, not a Maya scene instance. Preserve
        # distinct referenced instances by UUID, with exact DAG path as the
        # legacy fallback. Only old rows lacking both identities fall back to
        # the previous asset-level deduplication behavior.
        instance_identity = (
...
def _resolve_readable_video_reference(value: Any) -> Optional[Path]:
    """Resolve one Griptape/local reference and prove that it is readable."""
    ...
    *,
    ...
    for field, reference in (
        ...
        if _is_remote_video_reference(reference):
            ...
            return reference
        if _resolve_readable_video_reference(reference) is not None:
            if field == "video_path":
...
def _external_media_url(path: Path) -> str:
    ...
    try:
        ...
        if base_url:
            external_path = str(resolved).replace("\\", "/").lstrip("/")
